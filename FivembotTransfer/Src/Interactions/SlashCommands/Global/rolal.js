...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if(!member.roles.cache.has(role.id)) return interaction.reply({ content: "Rol zaten alınmış!", ephemeral: true });
        ...
        if(data.rollogalma == "notset") {
            
    }else{
        const embed = new EmbedBuilder()
        .setTitle("Rol Alındı")
        .setDescription(`${member} kullanıcısına ${role} rolü alındı.\nYetkili: ${interaction.user}`)
        .setColor(0xff2400)
        interaction.guild.channels.cache.get(data.rollogalma).send({ embeds: [embed] });
    }
    interaction.reply({ content: `${member} kullanıcısına ${role} rolü alındı.`, ephemeral: true });
